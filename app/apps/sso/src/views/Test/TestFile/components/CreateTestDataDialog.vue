    ...
    >
      <div v-if="variable.length > 0" class="el-card__body">
        <el-row :gutter="10">
          ...
          <el-col>
            <div class="mb-3" :class="!isMobile ? 'flex justify-between' : ''">
...
IF NOT ( [File system] = "NTFS" OR\n ( [File system] = "NTFS" AND NOT [Cluster size] <= 4096 )\nTHEN [Compression] = "Off";`,
  ...
  computed: {
    ...mapGetters(['selectedProjectId', 'selectedProject', 'userId', 'userRole', 'device']),
    isAllFill() {
  ...
  methods: {
    numberCheck (rule, value, callback) {
      ...
      if (!value.split(',').every((item) => /^\+?[1-9]\d*$/.test(item))) {
