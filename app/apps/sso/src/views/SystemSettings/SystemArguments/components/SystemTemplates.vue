<template>
  <div :class="isMobile ? '' : 'container'">
    <div v-if="!isLogs" v-loading="isLoading" :element-loading-text="loadingText">
      <div :class="isMobile ? 'text-xl' : 'text-2xl'">{{ $t('SystemTemplates.TemplatesSettings') }}</div>
      ...
      <el-row>
        <el-col :span="8">
          ...
          >
            <span v-if="!isMobile">{{ $t('SystemTemplates.ExecLogsButton') }}</span>
          </el-button>
        </el-col>
    ...
    <div v-if="isLogs">
      ...
      <span>
        ...
        <em v-if="is_lock" class="el-icon-loading font-bold text-danger" />
