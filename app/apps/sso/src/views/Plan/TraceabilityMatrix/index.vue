    ...
    <ProjectListSelector>
      ...
      >
        <el-form>
          ...
          <el-button
            class="w-full buttonPrimaryReverse"
            ...
            <el-col :span="22">
              ...
              <el-form-item>
                ...
                >
                  ...
                  >
                    <span v-if="!isMobile">{{ $t('Track.Download') }}</span>
                  </el-button>
                </el-popover>
              </el-form-item>
              <el-form-item>
                ...
                >
                  ...
                  >
                    <span v-if="!isMobile">{{ $t('IssueMatrix.ConditionSettings') }}</span>
                  </el-button>
                </el-popover>
              </el-form-item>
            </el-col>
        ...
        >
          ...
          <el-form-item>
            <el-button
              ...
              icon="el-icon-setting"
              :loading="isRunning()"
              :style="isMobile? 'padding: 7px;' : ''"
              @click="settingDialogVisible=!settingDialogVisible"
            >
              <span v-if="!isMobile">{{ $t('Track.CheckRuleSettings') }}</span>
            </el-button>
          </el-form-item>
          <el-form-item
            :label="$t('Track.CheckOrder')"
          ...
          >
            ...
            >
              {{ idx + 1 }}.
              <el-tag :size="isMobile ? 'mini' : 'medium'">
                <Tracker
                  :name="$t(`Issue.${item}`)"
                  :type="item"
                />
...
export default {
  ...
  computed: {
    ...mapGetters(['selectedProjectId', 'tracker', 'device']),
    startPoint() {
      ...
      return this.nowFilterValue.issueList
        .map((issue) => {
          const tracker = `Issue.${issue.tracker.name}`
          return `${this.$t(tracker)} #${issue.id} - ${issue.name}`
        })
        .join(', ')
    },
    getPercentProgress() {
      ...
      const nowVersionList = [
        ...new Set(
          this.chartIssueList.map((issue) =>
            issue?.fixed_version?.id ? issue.fixed_version.id : 'null'
          )
        )
      ]
  ...
  methods: {
    ...
    downloadSVG() {
      try {
        ...
        downloadLink.download = `TraceabilityMatrix_${getLocalTime(Date.now(), 'YYYY-MM-DD_HH:mm:ss')}`
...
$max_height: calc(100vh - 50px - 20px - 50px - 50px - 50px - 80px - 60px);
$max_width: calc(100vw);
.wrapper {
  height: #{$max_height};
  @apply static;
  &.hidden-filter-setting {
    height: calc(#{$max_height} + 100px);
  }
