  ...
  >
    <el-col>
      <el-row type="flex" align="middle">
        <el-col v-if="!isProjectSettingsDialog">
          <el-button
            type="text"
            size="medium"
            icon="el-icon-arrow-left"
            class="text-title linkTextColor mr-3"
            @click="handleBack"
          >
            {{ $t('general.Back') }}
          </el-button>
      ...
      <el-divider v-if="!isProjectSettingsDialog" />
      <div class="flex justify-between">
        ...
        <div v-if="!isProjectSettingsDialog">
          <el-button
      ...
      >
        <template
          ...
          v-slot:[`header-${tool.name}`]="{ props }"
        >
          <div :key="`header-${tool.name}-${idx}`">
...
export default {
      ...
      const columns = [
        {
          ...
          prop: 'branch',
          width: this.testingToolNames ? '100' : 'auto',
          align: 'center'
        },
        {
          label: this.$t('general.Description'),
          prop: 'commit_message',
          width: this.testingToolNames ? '120' : 'auto',
          align: 'center',
        ...
        },
        {
          ...
          prop: 'commit_time',
          width: this.testingToolNames ? '160' : 'auto',
          align: 'center',
          type: 'time'
        }
      ]
      this.testingToolNames?.forEach(tool => {
        columns.push({
          label: tool.name,
  ...
  methods: {
    ...
    handleBack() {
      this.$router.push({ name: 'ProjectSettingParent', params: { projectName: this.selectedProject.name }})
    },
    showWarningMessage() {
      this.$message({
