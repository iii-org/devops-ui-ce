...
import { getKeycloakUserInfo, logout } from '@/api_v2/user'
import { getJwtToken, removeToken, setToken } from '@/utils/auth'
...
const actions = {
  ...
  async getInfo({ commit, state, dispatch, rootState }) {
    const user = await (await getKeycloakUserInfo()).data
    ...
    commit('SET_USER_ROLE', user.default_role.name)
